...
void camera_proc(const uint8_t base)
{
	...
	if (player.x > 166)
	{
		...
		{
			camera.x += SC_VEL;
			if (camera.x % 8 == 0) set_attributed_bkg_submap(((camera.x>>3)-1) + DEVICE_SCREEN_WIDTH, (camera.y>>3), 1, 18, currlvl.map, currlvl.attr, currlvl.width, base);
			wait_vbl_done();
			SCX_REG = camera.x;
	...
	if (player.x <= -6)
	{
		...
		{
			camera.x -= SC_VEL;
			if (camera.x % 8 == 0) set_attributed_bkg_submap(camera.x>>3, camera.y>>3, 1, 18, currlvl.map, currlvl.attr, currlvl.width, base);
			wait_vbl_done();
			SCX_REG = camera.x;
	...
	if (player.y > 144)
	{
		...
		{
			camera.y += SC_VEL;
			if (camera.y % 8 == 0) set_attributed_bkg_submap(camera.x>>3, (camera.y>>3)-1 + DEVICE_SCREEN_HEIGHT, 20, 1, currlvl.map, currlvl.attr, currlvl.width, base);
			wait_vbl_done();
			SCY_REG = camera.y;
	...
	if (player.y <= 0)
	{
		...
		for (uint8_t i = 0; i < 144/SC_VEL; i++)
		{
			...
			if (camera.y % 8 == 0) set_attributed_bkg_submap(camera.x>>3, camera.y>>3, 20, 1, currlvl.map, currlvl.attr, currlvl.width, base);
			wait_vbl_done();
			SCY_REG = camera.y;
